...
/// The timezone names are cached to avoid re-fetching them
/// every time.
Iterable<String>? _timezoneNames;

/// Fetch all the timezone names from the database.
/// Some keys in the database are not actually timezone names
/// and are used for other purposes. These keys are filtered out.
Iterable<String> getTimeZonesNames() {
  if (_timezoneNames != null) {
    return _timezoneNames!;
  }
  // Remove the ignored keys from the timezone names
  _timezoneNames = _tzDatabase.keys.where(
    (e) => _ignoredKeys.every((k) => !k.hasMatch(e)),
  );
  return _timezoneNames!;
}

/// Cached timezones to avoid re-creating the timezone every time.
final _timezones = <String, UniversalTimezone>{};

/// Fetch a timezone by its name.
UniversalTimezone getTimeZoneByName(String name) {
  if (!getTimeZonesNames().contains(name)) {
    throw TimezoneNotFoundException(name);
  }

  /// If the timezone is already cached, return it.
  if (_timezones.containsKey(name)) {
    return _timezones[name]!;
  }

  /// Fetch the raw timezone data from the database.
  var rawTimezoneData = _tzDatabase[name]!;
  ...
  if (!RegExp(r"^[+-]").hasMatch(rawTimezoneData)) {
  ...
  _timezones[name] = timezone;
  return timezone;
}

@immutable
class UniversalTimezone extends BaseTimezone with EquatableMixin {
  @override
  final String id;
  ...
  (_LocalTimeType, int) _localTimeFor(int millisecondsSinceEpoch) {
    ...
    for (var (index, delta) in _transitionDeltas.indexed.skip(1)) {
  ...
  int offset(int millisecondsSinceEpoch) {
    ...
    /// If we are after the last trransition, then we must use the [_dstRule]
    /// to infer the offset.
    if (_lttIndex60 != null &&
    ...
    /// If we are before the 1st transition, we return
    /// whatever the current offset is, without the DST offset.
    // if (transitionIndex == 0) {
    //   final (localTimeNow, nowIndex) =
    //       _localTimeFor(DateTime.timestamp().millisecondsSinceEpoch);

    //   return Offset(
    //     offset: localTimeNow.utcOffset60 - localTimeNow.dstOffset60,
    //     dstDelta: null,
    //   );
    // }

    // if (transitionIndex == 0 ||
    //     transitionIndex == _transitionDeltas!.length - 1) {

    //   var offset = localTimeType.utcOffset60;
    //   final transitions = transitionsFor(DateTime.fromMillisecondsSinceEpoch(
    //           millisecondsSinceEpoch,
    //           isUtc: true)
    //       .year);
    //   if (transitions == null) {
    //     return Offset(offset: offset, dstDelta: null);
    //   }
    //   final ruleA = transitions.$1;
    //   final ruleB = transitions.$2;
    //   final firstRule = (ruleA.start < ruleB.start ? ruleA : ruleB);
    //   final secondRule = (ruleA.start > ruleB.start ? ruleA : ruleB);

    //   if (millisecondsSinceEpoch >= firstRule.start &&
    //       millisecondsSinceEpoch < secondRule.start) {
    //     return Offset(
    //         offset: offset + firstRule.offset, dstDelta: firstRule.offset);
    //   } else {
    //     return Offset(
    //         offset: offset + secondRule.offset, dstDelta: secondRule.offset);
    //   }
    // }
  }
...
class _DstRule {
  ...
  int transitionForYear(int year, int stdOffset, int dstOffset) {
    ...
    if (_dayOfWeek >= 0 && _dayOfMonth != 0) {
      ...
      // the dayOfWeek.
      var tempDate =
          DateTime.utc(year, _month + 1, 1).subtract(const Duration(days: 1));
      while (tempDate.weekday != effectiveDayOfWeek) {
...
class _Basic {
  ...
  factory _Basic(String text) {
    ...
    return _Basic._(
      initialUtcOffset: parseHHMMorHHMMSS(parts[0]),
      currentStdUtcOffset: parseHHMMorHHMMSS(parts[1]),
...
class _LocalTimeType with EquatableMixin {
  ...
  factory _LocalTimeType(String text) {
    ...
    return _LocalTimeType._(
      utcOffset60: base60.parseDuration(parts[0]),
      dstOffset60: base60.parseDuration(parts[1]),
      abbreviation: parts.length > 2 ? parts[2] : null,
    );
  }
  static List<_LocalTimeType> parseList(String text) {
    return text.split(' ').map((e) => _LocalTimeType(e)).toList();
  }
...
/// See https://github.com/kshetline/tubular_time_tzdb#timezone-descriptions for more information.
int parseHHMMorHHMMSS(String input) {
...
final base60 = _Base60Encoder();

/// Handy extension to convert empty lists to null.
extension _NullIfEmpty<T> on List<T>? {
  List<T>? get nullIfEmpty => this?.isEmpty == true ? null : this;
}

extension _NullIfBlank on String? {
  String? get nullIfBlank => this?.isNotEmpty == true ? this : null;
}
